...
### Calendar Variants
| 7-Day Row Grid (Default) | Vertical Column (Left) | Vertical Column (Right) |
| :---: | :---: | :---: |
| <img src="assets/calendar_grid.png" width="280" alt="Calendar Grid"/> | <img src="assets/calendar_column.png" width="140" alt="Calendar Column Left"/> | <img src="assets/calendar_column_right.png" width="140" alt="Calendar Column Right"/> |
...
| :---: |
| <img src="assets/stats.png" width="260" alt="System Stats"/> |
...
| :--- | :--- | :--- |
| **Start** | `./control.sh st` | Start all widgets (`st clo`, `st cal`, `st sys`, `st stats`) |
| **Stop** | `./control.sh sp` | Stop all widgets (`sp clo`, `sp cal`, `sp sys`, `sp stats`) |
...
./control.sh cal col-left          # Vertical Column on Left side (under month)
./control.sh cal col-right         # Vertical Column on Right side (under weekday)
...
- **Stats**: `widgets/stats/stats.conf`

### 1. Colors
Edit the hex values in any `.conf` file:
```lua
color1 = 'E63946',  -- Accent (AM/PM, weekday, today's date, system slash, stats accents)
color2 = 'F5F5F7',  -- Primary (clock digits, upcoming dates, hostname, metric values)
color3 = '8D99AE',  -- Muted (month title, past dates, OS subtitle, labels)
```
...
- **Stats** (`widgets/stats/stats.conf`):
  ```lua
  alignment = 'bottom_left', -- Bottom left corner of the screen
  gap_x     = 40,            -- Aligned flush with Clock & Calendar
  gap_y     = 40,            -- Distance from bottom edge
  ```
